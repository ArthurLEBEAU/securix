...
export default class PageController {
  ...
  async service(req, res) {
    ...
    if (categories) {
      ...
      active_category = categories.filter(
        (it, i) => it.name == active_category
      );
      if (active_category.length == 0) active_category = "all";
      const filter =
        active_category == "all" ? {} : { cat_id: active_category[0]._id };
      active_category =
        active_category == "all" ? "all" : active_category[0].name;
  ...
  async single(req, res) {
    try {
      const article = await Blog.findOne({ _id: req.params.id }).populate(
        "cat_id",
        "name + _id"
      );

      if (article != null) {
        let articles = await Blog.find({ cat_id: article.cat_id._id }).populate(
          "cat_id",
          "name + _id"
        );
        res.status(HttpResponse.OK);
   ...
   * @returns Express.res
   */
  async comparator(req, res) {
    res.status(HttpResponse.OK);
    return res.render("global/comparateur.njk");
  }
