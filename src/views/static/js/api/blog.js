...
$(document).ready(async function() {
    const _url = 'http://localhost:3000/api/article/';
    let articleDataTable = $('#articlesTable').DataTable({
        ajax: {
            url: _url,
            cache: true,
            dataSrc: "articles"
        ...
        columnDefs: [{
            ...
            render: function(data, type, row) {
                return `<img class='img-thumbnail img-fluid' alt='couverture de l'article' src='/static${data}' />`;
            },
            targets: 1,
        }],
    ...
    $(document).on('click', '.btn_update', (e) => {
            ...
            for (let index = 0; index < articleDataTable.rows()[0].length; index++) {
                const element = (articleDataTable.rows(index).data())[0];
                if (element._id == current_id) {
                    $('#title').val(element.title);
                    $('#cover_input').attr("src", "/static" + element.cover);
                    $('#category').val(element.categorie._id);
                    $('#resume').val(element.resume);
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    data: { id: id },
                    success: function() {
                        articleDataTable.row(target).remove().draw();
    ...
    $('#form_modal').submit(function(e) {
        ...
        if (cat_id == "categorie") {
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: "choisisser une categorie s'il vous plait !",
            })
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    Swal.fire({
                        ...
                        // reload data
                    articleDataTable.ajax.reload(null, false);
                    // reset the form
